...
function pintarCursos(listaCursos) {
  ...
  listaCursos.forEach((curso) => {
    let htmlCurso = `
        <div class="card">
          <img src="img/${curso.img}" width="150px" height="150px" class="imagen-curso">
          <div class="info-card">
              <h4>${curso.title}</h4>
              <p>Marca: ${curso.brand}</p>
              <p>Velocidad Máxima: ${curso.velocity} km/h</p>
              <p>Capacidad Máxima: ${curso.capacity} mAh</p>
...
function generarCarritoHTML() {
  ...
  articulosCarrito.forEach((curso) => {
    const {titulo, precio, cantidad, id } = curso; // Usamos destructuring
...
function searchCursos(e) {
  ...
  const brand = document.getElementById("buscador").value;
  cargarCursos(title);
  cargarCursos(brand);
}
...
function mostrarAlert() {
  Swal.fire({
    ...
    icon: "success",
    title: "Patinete añadido al carrito",
